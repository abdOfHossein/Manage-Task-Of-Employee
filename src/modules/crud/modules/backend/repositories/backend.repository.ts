...
export class BackendRepo {
    ...
    async _createEntity(createDto: CreateBackendDto, query?: QueryRunner): Promise<BackendEnt> {
    ...
    async _updateEntity(entity: BackendEnt, updateDto: UpdateBackendDto, query?: QueryRunner): Promise<BackendEnt> {
    ...
    async _deleteEntity(entity: BackendEnt, query?: QueryRunner): Promise<BackendEnt> {
      if(query) return await query.manager.remove(entity);
      return await this.dataSource.manager.remove(entity);
    }
    async _paginationEntity(pageDto: BackendPageDto): Promise<PageDto<BackendEnt>> {
      const queryBuilder = this.dataSource.manager.createQueryBuilder(BackendEnt, 'backend')
      if (pageDto.base) {

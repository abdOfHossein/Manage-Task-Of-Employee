...
export class FrontendRepo {
  constructor(private dataSource: DataSource) {
   
}
    async _paginationEntity(pageDto: FrontendPageDto): Promise<PageDto<FrontendEnt>> {
        const queryBuilder = this.dataSource.manager.createQueryBuilder(FrontendEnt, 'frontend')
  if (pageDto.base) {
  ...
  if (pageDto.field) {
    ...
    if (!mapper) throw new Error(`${JSON.stringify({ section: "public", value: 'Route Already Exist'})}`)
  ...
  return new PageDto(result[0], pageMetaDto);
    ...
    async _findOneEntity(searchDto: string, options?: FindOneOptions<any>): Promise<FrontendEnt> {
        return await this.dataSource.manager.findOne(FrontendEnt, { where : {id : searchDto } ,relations: ['menu']})
    }
    async _createEntity(createDto: CreateFrontendDto, query?: QueryRunner): Promise<FrontendEnt> {
    ...
    async _updateEntity(entity: FrontendEnt, updateDto: UpdateFrontendDto, query?: QueryRunner): Promise<FrontendEnt> {
    ...
    async _deleteEntity(entity: FrontendEnt, query?: QueryRunner): Promise<FrontendEnt> {
      if (query) return await query.manager.save(entity)
      return await this.dataSource.manager.save(entity)
    }

    async findByRoute(route: string) {
      return await this.dataSource.manager.findOne(FrontendEnt, { where: { route } })
    }
}